...
Thank you for improving `aws.modules.acm`. This guide covers the toolchain, the local quality gate, how features are tested and where they belong, commit and pull request conventions, and how releases are cut.
...
make integration-smoke   # about a minute; one pending public certificate, requested and deleted
```

Add a suite when a feature's correctness depends on the AWS API rather than on rendering (for example a new validation shape or a private CA path). Keep every value derived from the environment or from disposable fixtures the suite creates, and never reference a real zone, CA, or account. A suite that needs fixtures keeps them in `tests/integration/setup`, which the policy scans exclude.
...
- Tests live in `tests/*.tftest.hcl`, one file per concern: `defaults`, `dns_validation`, `email_validation`, `private_ca`, `imported`, and `validation` (every variable validation and precondition). Each file starts with `mock_provider "aws" {}` and a `variables` block holding a valid baseline; each `run` overrides only what it exercises.
- Use `command = plan`. Nothing here talks to AWS, so tests run in seconds and in CI without credentials.
- Validations are tested with `expect_failures`. Point it at the object that carries the check: `[var.domain_name]` for a variable validation, `[aws_acm_certificate.this]` for a precondition on the certificate, `[check.certificate_transparency_disabled]` for a `check` block. A run with `expect_failures` passes only if exactly those objects fail; add a positive run alongside so the happy path is covered too.
- Assertions must not depend on unknown values. With a mock provider, computed attributes such as ARNs, IDs, and the certificate's `domain_validation_options` are unknown at plan time. Assert on what the module knows by construction: the record `for_each` keys (`keys(aws_route53_record.validation)`), each record's `zone_id`, `ttl`, and `allow_overwrite`, resource counts, the wait resource and its `timeouts.create`, tags, and outputs derived from inputs such as `mode` and `validation_record_zone_ids`. Never assert on a record's `name`, `type`, or `records`.
...
- `imported` is sensitive. The mode it selects is derived with `nonsensitive()` on the presence check only, never on the material; a test that touches import values compares the certificate arguments directly, which the test runner permits.
...
| Concern | Lives in |
| --- | --- |
| A certificate argument (a new ACM request or import option) | `variables.tf` with a description, type, and validation; `certificate.tf` to render it, guarded by the mode it belongs to; a precondition when it must be rejected in the other modes; a test in that mode's test file and an `expect_failures` run in `tests/validation.tftest.hcl`. |
| Mode resolution, the domain list, zone selection, wildcard handling | `locals.tf`, with the behaviour pinned in `tests/dns_validation.tftest.hcl`. |
| Validation records and the wait for issuance | `validation.tf`. |
| Cross-input rules | Preconditions on `aws_acm_certificate.this` in `certificate.tf`, or `checks.tf` when the situation is valid but usually unintended. |
| Outputs | `outputs.tf`; every output has a description, and one that is derived from inputs is asserted in a test. |

Rules that apply everywhere: no data sources (derive from inputs and the provider's region), every variable has a description, a type, and a validation where a wrong value would otherwise fail at apply time, every output has a description, defaults are the secure choice, and an input that applies to one mode is rejected in the others rather than ignored.
...
```text
feat(validation): accept a per-record TTL override
fix(locals): fold a wildcard into its apex record
docs: explain external validation through another DNS provider
test(email): cover validation_option for a parent domain
feat!: replace route53_zone_id with the route53_zones map
```
...
2. Create a signed annotated tag on the merge commit. The signing key must be registered with GitHub so the tag shows as Verified:
   ...
   ```sh
   git tag -s vX.Y.Z -m "aws.modules.acm vX.Y.Z"
...
4. Announce the release with the commit SHA. Consumers pin that SHA, not the tag:
   ...
   ```hcl
   source = "git::https://github.com/hatan4ik/aws.modules.acm.git?ref=<commit-sha>" # vX.Y.Z
   ```
